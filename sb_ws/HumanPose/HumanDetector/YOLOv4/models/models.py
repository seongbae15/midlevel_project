    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef["type"] == "convolutional":
            ...
            if isinstance(k, int):  # single-size conv
                ...
                modules.add_module(
                    "MixConv2d", MixConv2d(in_ch=output_filters[-1], out_ch=filters, k=k, stride=stride, bias=not bn)
                )
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            p_cls = (
                torch.sigmoid(p[:, 4:5]) if self.nc == 1 else torch.sigmoid(p[:, 5 : self.no]) * torch.sigmoid(p[:, 4:5])
            )  # conf
